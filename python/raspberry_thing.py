...
def get_b64_auth():
    """
    Get the base 64 auth string.
    """
    userpw_string = THINGS_USER + ":" + THINGS_PASSWORD
...
class RaspberryDemoThing:
    ...
    def __poll_temperature_values(self, callback):
        while True:
            try:
                # read sensor
                [temperature, humidity] = self.tempSensor.get_temperature_and_humidity()
                # call callback
                callback(temperature, humidity)
            except Exception as e:
                print('Error when providing temperature/humidity values. Trying again')
                raise e
            finally:
                # wait for next read
